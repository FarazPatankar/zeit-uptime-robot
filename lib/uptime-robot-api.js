...
const { parseMonitorName, generateRandomString } = require('./utils');
...
const addNewMonitor = async (key, name, { monitorName, monitorUrl }, alertContacts) => {
  const slug = monitorName ? parseMonitorName(monitorName) : generateRandomString();
  const friendlyName = name + "-" + slug;
...
const createNewPSP = async (key, projectName, projectMonitors) => {
  ...
  const monitorsForProject = typeof projectMonitors === "number"
    ? '0'
...
const deletePSP = async (key, pspId) => {

  const response = await fetch(UPTIME_ROBOT_BASE_URL + DELETE_PSP_URL, {
    headers,
    method: 'POST',
    body: `api_key=${key}&id=${pspId}&format=json`,
  });

  const { psp } = await response.json();
  console.log('delete psp', psp);

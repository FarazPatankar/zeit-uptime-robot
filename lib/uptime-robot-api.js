...
const addAlertContact = async (key, email) => {
  ...
  const response = await fetch(UPTIME_ROBOT_BASE_URL + ADD_CONTACT_URL, {
    headers,
    method: 'POST',
    body: `api_key=${key}&friendly_name=${email}&value=${email}&type=${type}&format=json`,
  });
...
const addNewMonitor = async (key, name, monitorUrl, alertContacts) => {
  ...
  console.log('ac', alertContactsString);
...
module.exports = { getAccountDetails, addNewMonitor, getMonitors, pauseMonitor, deleteMonitor, resumeMonitor, getAlertContacts, addAlertContact };
